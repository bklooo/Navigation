...
class Navbot7Spider(scrapy.Spider):
    name = 'navbot_7'
    start_urls = ['http://top.chinaz.com/hangye/index_wangluo_jishu.html/']

    def parse(self, response):
        item = SitespiderItem()
        if(response.status == 200):
            for i in response.xpath('//*[@id="content"]/div[4]/div[3]/div[2]/ul/li[position()>2]'):
