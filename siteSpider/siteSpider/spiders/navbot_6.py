...
class Navbot6Spider(scrapy.Spider):
    name = 'navbot_6'
    start_urls = ['https://top.chinaz.com/hangye/index_wangluo_sheji.html/']

    def parse(self, response):
        item = SitespiderItem()
        if(response.status == 200):
            for i in response.xpath('//*[@id="content"]/div[4]/div[3]/div[2]/ul/li[position()>1]'):
